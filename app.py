...
def print_receipt(txn_id):
    ...
    <body>
        <div class="header">
            ...
            <p style="font-size:11px; color:#64748b; margin-top:2px;">NAGAHEE KAFFALTII (OFFICIAL RECEIPT)</p>
        </div>

        <div class="row"><span>Ref ID:</span> <b>{t['ft_reference']}</b></div>
        <div class="row"><span>Guyyaa:</span> <span>{t['timestamp']}</span></div>
        <div class="row"><span>Gosa Transaction:</span> <b>{t['txn_type']}</b></div>
        <div class="row"><span>Maammila:</span> <b>{t['customer_name']} ({t['customer_id']})</b></div>
        {f'<div class="row"><span>Target Acc:</span> <b>{t["target_account"]} ({target_name})</b></div>' if t['target_account'] else ''}
        <div class="row" style="font-size:16px; font-weight:bold; color:#065f46; border-top:1px dashed #ccc; pt:8px; margin-top:8px;">
            <span>Hamma Maallaqaa:</span> <span>{float(t['amount']):,.2f} Birr</span>
        </div>
        ...
        <div style="margin-top:30px; text-align:center; font-size:11px; color:#64748b;">
            ...
            <p style="margin-top:15px;">________________________<br>Mallattoo / Signature</p>
        </div>
...
def manage_users():
    ...
    if request.method == 'POST':
        ...
        if action_type == 'add':
            ...
            if cursor.fetchone():
                msg = f"❌ User '{username}' duraan jira!"
            else:
            ...
            if target_user != 'ceo':
                ...
                conn.commit()
                msg = f"✅ User '{target_user}' status {new_status}-tti jijjiirameera."
    ...
    for u in users_list:
        ...
        if u['username'] != 'ceo':
            ...
            btn_txt = "🚫 Ugguri (Block)" if u['status'] == 'ACTIVE' else "🔓 Uggura Kaasi"
            status_btn = f"""
        ...
        <div class="item-card" style="display:flex; justify-content:space-between; align-items:center;">
            <div>
                ...
                <span class="badge {'badge-active' if u['status']=='ACTIVE' else 'badge-danger'}" style="margin-left:6px;">{u['status']}</span>
            </div>
            <div>{status_btn}</div>
        </div>
        """
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select name="role" class="input-field">
                    ...
                    <option value="CEO">CEO (Bulchaa Preezidantii)</option>
                </select>
            </div>
            <button type="submit" class="btn-submit" style="background:#6b21a8;">➕ User Haaraa Uumi</button>
        </form>
    </div>

    <h3 style="font-size: 14px; margin-bottom: 8px; color: #334155;">👥 User-oota Systema Irratti Jiran</h3>
    {users_html}
    """
    return render_template_string(HTML_LAYOUT.replace("{% block content %}{% endblock %}", content), notifications=NOTIFICATIONS)
